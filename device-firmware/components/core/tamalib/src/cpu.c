 ...
 */
/*
 * Modified 2026-08-27 for the ESP32-C3 port:
 * - initialize the decoded-op index for the halted path;
 * - build a 12-bit opcode lookup once, avoiding a linear scan through 108
 *   instruction descriptors on every emulated instruction.
 */
#include "cpu.h"
#include "hw.h"
#include "hal.h"
...
#define OP_INDEX_INVALID 0xFFu
static u8_t op_index[0x1000];
static bool_t op_index_ready;

static void build_op_index(void)
{
	for (unsigned opcode = 0; opcode < 0x1000; opcode++) {
		u8_t index = 0;
		while (ops[index].log != NULL &&
		       (opcode & ops[index].mask) != ops[index].code) {
			index++;
		}
		op_index[opcode] = ops[index].log != NULL ? index : OP_INDEX_INVALID;
	}
	op_index_ready = 1;
}
...
static timestamp_t wait_for_cycles(timestamp_t since, u8_t cycles) {
	...
	if (speed_ratio == 0) {
		/*
		 * Emulation will be as fast as possible. No wall-clock deadline is
		 * consumed until normal speed is restored and cpu_sync_ref_timestamp()
		 * is called, so reading the ESP timer for every instruction only wastes
		 * cycles during offline catch-up.
		 */
		return since;
	}
...
int cpu_step(void)
{
	...
	if (!cpu_halted) {
		...
		/* Direct lookup replaces the original per-step linear descriptor scan. */
		i = op_index[op & 0x0FFF];
		if (i == OP_INDEX_INVALID) {
