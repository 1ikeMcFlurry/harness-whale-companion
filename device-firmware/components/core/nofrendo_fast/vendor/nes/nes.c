...
void __attribute__((section(".iram1"))) nes_renderframe(bool draw_flag)
{
   int elapsed_cycles;
   mapintf_t *mapintf = nes.mmc->intf;
   bitmap_t *framebuffer = vid_getbuffer();
   int in_vblank = 0;

   while (262 != nes.scanline)
   {
//      ppu_scanline(nes.vidbuf, nes.scanline, draw_flag);
		ppu_scanline(framebuffer, nes.scanline, draw_flag);

#ifdef NOFRENDO_EMBEDDED
      if (draw_flag && nes.scanline < NES_SCREEN_HEIGHT &&
          (nes.scanline + 1) % NOFRENDO_STREAM_LINES == 0)
      {
         extern void nofrendo_fast_scanlines_ready(unsigned first_line,
                                                    unsigned line_count);
         nofrendo_fast_scanlines_ready((unsigned)(nes.scanline + 1 -
                                       NOFRENDO_STREAM_LINES),
                                       NOFRENDO_STREAM_LINES);
      }
#endif
